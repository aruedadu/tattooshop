...
public interface ArtistaRepositoryJPA extends JpaRepository<ArtistEntity, Long> {
	...
	@Query("SELECT a FROM ArtistEntity a WHERE a.id NOT IN (SELECT p.tattooArtist.id FROM AppointmentEntity p WHERE (p.startDate BETWEEN :FECHA_INI AND :FECHA_FIN)"
